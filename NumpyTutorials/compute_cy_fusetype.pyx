...
cdef arrType clip(arrType a, arrType min_value, arrType max_value):
...
def compute(arrType[:,::1] array_1, arrType[:,::1] array_2, int a, int b, int c):
    """
    ...
    if arrType is int:
        DTYPE = np.intc
    elif arrType is double:
        DTYPE = np.double
    elif arrType is cython.longlong:
        DTYPE = np.longlong
    ...
    cdef arrType[:,:] result_view = result
